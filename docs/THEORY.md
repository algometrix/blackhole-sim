...
10. [What a distant observer sees fall in](#10-what-a-distant-observer-sees-fall-in)
...
16. [What we cheat on](#16-what-we-cheat-on)
17. [Every equation and where it lives](#every-equation-and-where-it-lives)
...
| **No light travel-time delay.** | You see the whole disc at one instant rather than each part as it was when its light left. The infalling probe is drawn at its current Schwarzschild coordinate position too, with propagation treated as instantaneous: that changes the time constant of the freeze, not the fact of it. | Track photon arrival times through the march. |
...
| Gravitational redshift | $\sqrt{1 - r_s/r}$ | `sim/beacon.ts` |
| Radial infall energy | $E = \sqrt{1 - r_s/r_0}$ | `sim/beacon.ts` |
| Infaller redshift | $g = f/(E + \sqrt{E^2 - f})$ | `sim/beacon.ts` |
| Coordinate infall rate | $dr/dt = -f\sqrt{E^2 - f}\,/\,E$ | `sim/beacon.ts` |
| Proper time to the horizon | $\tau = \tfrac{1}{2}\sqrt{r_0^3/r_s}\,(\eta + \sin\eta)$ | `sim/beacon.ts` |
| Apparent image radius | $b = r/\sqrt{1 - r_s/r}$, floored at $b_{\text{crit}}$ | `sim/beacon.ts` |
